      ...
      switch (animationState) {
        ...
        case "vaporizing": {
          ...
          if (vaporizeProgressRef.current >= 130) {
            onCycleComplete?.(currentTextIndex);
...
const createParticles = (
  ...
  // Calculate sampling rate so particle count stays bounded regardless of
  // canvas resolution (fixed strides on a large/high-DPR canvas produce
  // hundreds of thousands of particles and tank frame rate).
  ...
  for (let y = 0; y < canvas.height; y += sampleRate) {
    for (let x = 0; x < canvas.width; x += sampleRate) {
      ...
      if (alpha > 0) {
        // Remove density from opacity calculation, boosted so the sparse
        // sampling still reads as solid white rather than washed-out gray
        const originalAlpha = Math.min(1, (alpha / 255) * (sampleRate / currentDPR) * 1.8);
        const particle = {
          x,
          y,
          originalX: x,
